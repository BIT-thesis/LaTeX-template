...
公式定义的内容包含在\textbackslash begin\{equation\}和\textbackslash end\{equation\}之间。为方便，公式的编辑可以采用在线的\LaTeX{}公式编辑器。公式的编号格式可以在./sty文件夹下的格式文件中定义，公式中涉及的术语表也需要在公式后面进行对应标记。一般的\LaTeX{}编辑器如Winedit、texmaker等都有公式编辑器。

{\bf{实例1：}} 以下是L-B非稳态流动升力模型，公式引用为~\ref{eqn:LBmodel}。
\begin{equation}
 \label{eqn:LBmodel}
...
\footnote{当然，使用者也可以手动编参考文献item，直接插入文档中。但是，有BibTeX帮助，处理起参考文献更为简单}。
...
各大数据库\footnote{应该说是国际知名数据库，譬如~SCOPUS, IEEE, OSA等，国内数据库在搜索、导出方面效果不佳。}也支持将参考文献信息导出为.bib，
省时省力。
